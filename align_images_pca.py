...
import glob
import os
#src_dir_name = 'd:/dane/credo/png/'
#dst_dir_name = 'd:/dane/credo/png_align_image/'

src_dir_name = 'd:\\dane\\credo\\nowe\\wybrane\\'
dst_dir_name = 'd:\\dane\\credo\\nowe\\wybrane_align2\\'

# Get list of all files in a given directory sorted by name
list_of_files = sorted( filter( os.path.isfile,
                        glob.glob(src_dir_name + '*') ) )
for fil in list_of_files:
    srcI = cv2.imread(fil)
    shifted = align_image(srcI, mask=True)
    shifted = cv2.cvtColor(shifted, cv2.COLOR_BGR2GRAY)
    cv2.imwrite(dst_dir_name + "/" + os.path.basename(fil), shifted)
